...
import { FileText, Calendar, Layers, RefreshCw, Loader2, MessageSquare, Trash2, ChevronDown, ChevronUp, FolderOpen, FileUp } from 'lucide-react';
...
export function DocumentLibrary() {
  ...
  const handleToggle = async (id: number, currentStatus: boolean) => {
    ...
    try {
      ...
      // After toggling, reset agent memory so stale context is cleared
  ...
  const handleDelete = async (id: number, filename: string) => {
    ...
    try {
      ...
      // After deletion, reset agent memory to remove any references to the file
      await api.resetSession(sessionId);
    } catch (err) {
      fetchDocs();
    }
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-black/20 p-4 sm:p-6 rounded-2xl backdrop-blur-md border border-white/10 shadow-lg gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 border border-blue-400/30 text-white text-sm font-medium rounded-lg transition-all shadow-[inset_0_1px_1px_rgba(255,255,255,0.3)] hover:shadow-[0_0_15px_rgba(59,130,246,0.3)]"
          >
          ...
          <button
            onClick={fetchDocs}
            className="flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 border border-white/20 text-white text-sm font-medium rounded-lg transition-all shadow-[inset_0_1px_1px_rgba(255,255,255,0.3)] hover:shadow-[0_0_15px_rgba(255,255,255,0.1)]"
          >
